...
## Service worker caching (`frontend/cacheServiceWorker.js`, `.githooks/pre-commit`)
...
- A user-facing offline/online mode toggle - real service-worker caching is in place now (see
  "Service worker caching" above), but there's still no UI telling a user they're offline/what that
  means for the app.
...
`push`/`notificationclick` listeners — both independent of the cache-first `fetch` handler (see
"Service worker caching" above), so they work the same regardless of `BYPASS_CACHE` or cache state.
